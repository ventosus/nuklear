...
nk_cairo_get_text_width(nk_handle handle, float height, const char *text, int len)
{
    ...
    cairo_select_font_face(cairo, "cairo:monospace", CAIRO_FONT_SLANT_NORMAL,
...
nk_cairo_rectangle(cairo_t *cairo, double x, double y, double w, double h, double r)
{
    ...
    cairo_arc(cairo, x + w - r, y + r, r,     -NK_PI/2, 0.0);
    cairo_arc(cairo, x + w - r, y + h - r, r,  0.0,     NK_PI/2);
    cairo_arc(cairo, x + r,     y + h - r, r,  NK_PI/2, NK_PI);
    cairo_arc(cairo, x + r,     y + r, r,      NK_PI,   3*NK_PI/2);
...
nk_cairo_circle(cairo_t *cairo, double x, double y, double w, double h)
{
    ...
    cairo_arc(cairo, 0.0, 0.0, 1.0, 0.0, 2*NK_PI);
    cairo_restore(cairo);
}
